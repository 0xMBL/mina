...
|}
    ...
    let create ~image ~command ~entrypoint ~ports ~environment ~volumes
        ~networks ~dns =
      { image; command; entrypoint; ports; environment; volumes; networks; dns }
    ...
    let to_yojson
      ...
      `Assoc
        ( [ ("image", `String image)
          ...
          ; ("networks", `List (List.map ~f:Network.to_yojson networks))
